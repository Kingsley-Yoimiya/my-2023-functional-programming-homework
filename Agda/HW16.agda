...
open import Data.Vec using (Vec; []; _∷_; replicate; take; drop; last; head; insert; map; _⊛_)
...
zero-matrix 0 _ = []
zero-matrix (suc n) m = replicate 0 ∷ zero-matrix n m
...
module Problem-20-2-b where
  ...
  matrix-elt : {n m : ℕ}
    ...
    → j < m ≡ true
    → ℕ
  matrix-elt {suc n} {suc m} mat 0 0 p1 p2 = head (head mat)
  matrix-elt {suc n} {suc m} mat 0 (suc j) p1 p2 = matrix-elt ((drop 1 (head mat)) ∷ []) 0 j (refl) (p2)
  matrix-elt {suc n} mat (suc i) j p1 p2 = matrix-elt (drop 1 mat) i j (p1) p2
  matrix-elt {n} {zero} mat i j p1 p2 = j
  matrix-elt {zero} {suc m} mat i j p1 p2 = j
...
diagonal-matrix zero d = []
diagonal-matrix (suc n) d = (1 ∷ (replicate zero)) ∷ (map (0 ∷_ ) (diagonal-matrix n d))
...
transpose []         = replicate []
transpose mat@(as ∷ ass) = replicate _∷_ ⊛ as ⊛ transpose ass
